...
class Problem:
    ...
    def __init__(self, initial, goal=None,m=0,n=0):
        """The constructor specifies the initial state, and possibly a goal
        state, if there is a unique goal. Your subclass's constructor can add
        other arguments."""
        self.initial = initial
        self.goal = goal
        dimensioniMatrice=(m,n)
    ...
    def goal_test(self, state):
        ...
        if isinstance(self.goal, list):
            return any(is_in(state, self.goal))
        else:
            return any(state == self.goal)
...
class Node:
    ...
    def __repr__(self):
        stringaLista=self.stampa_lista(self.state,dimensioniMatrice[0],dimensioniMatrice[1])
        return stringaLista

    def __lt__(self, node):
        #print(f"__lt__ metod {self.path_cost} < {node.path_cost}")
        #print(f"__lt__ metod {self.state} < {node.state}")
        return self.path_cost < node.path_cost
    
    def stampa_lista(self,lista,n,m):
        stringa="\nNodo:\n[\n"
        for i in range(0,n):
            for j in range(0,m):
                stringa+=f"\t{lista[m*i+j]}\t"
            stringa+="\n"

        stringa+="]\n"
        stringa+="azione: "+str(self.action)+"\n"
        stringa+="costo: "+str(self.path_cost)+"\n"
        stringa+="profondità nodo: "+str(self.depth)+"\n"
        return stringa
    ...
    def expand(self, problem):
        ...
        return [self.child_node(problem, action, color)
                for action,color in problem.actions(self)]

    def child_node(self, problem, action, color):
        """[Figure 3.10]"""
        next_state = problem.result(self.state, (action,color))
        next_node = Node(next_state, self, (action,color), problem.path_cost(self.path_cost, self.state, (action,color), next_state))
        return next_node

    def solution(self):
        """Return the sequence of actions to go from the root to this node."""
        return [list(node.state) for node in self.path()[1:]]
